...
# MainActor를 호출할 때 await을 요구하는 이유

`@MainActor`는 UI 작업이나 메인 스레드에서 실행되어야 하는 코드를 관리하는 데 사용되는 Actor다.

그런데 MainActor에 정의한 함수들은 비동기 함수가 아닌데도, await을 사용해서 호출하라고 컴파일러가 알려준다.

그 이유는 다른 스레드나 비동기 컨텍스트에서 MainActor 함수를 호출할 때, 메인 스레드로 안전하게 전환하기 위해서다.
...
그래서 호출하는 함수가 동기 함수임에도 await으로 호출하는 것이 어색할 수 있지만, 이는 Swift Concurrency에서 메인 스레드에 안전하게 접근하고 코드를 실행하기 위한 요구사항에 해당한다.
...
위 함수는 동기 함수에서 MainActor 함수를 호출하는 케이스다.

이 때는 컴파일러가 두 가지 해결책을 제시해준다.

첫 번째는 위의 예시처럼 async 함수로 만들고 await으로 호출하라는 것이고,

두 번째는 이 함수도 MainActor로 만들라는 것이다.

이전 예시에서 얘기한 것처럼 await은 비동기 컨텍스트에서 안전하게 메인 스레드로 전환하기 위함인데,

이 함수 자체를 MainActor로 만들어버리면, 컴파일러가 이 함수가 메인 스레드에서 호출되는 것을 확실할 수 있기 때문에 문제가 해결된다. 
